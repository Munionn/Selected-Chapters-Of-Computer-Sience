...
class Certificate(models.Model):
    ...
    number = models.CharField('Номер сертификата', max_length=100)
    issue_date = models.DateField('Дата выдачи')
    expiry_date = models.DateField('Дата окончания', null=True, blank=True)
    issuing_authority = models.CharField('Кем выдан', max_length=200)
    description = models.TextField('Описание')
    image = models.ImageField('Изображение сертификата', upload_to='certificates/', null=True, blank=True)
    file = models.FileField('Файл сертификата', upload_to='certificates/files/', null=True, blank=True)
